...
// ✅ PERF: cache color per name so it's not recomputed on every render
...
// ✅ PERF: Avatar memoized — won't re-render unless src/name/online changes
const Avatar = memo(({ src, name, size = 42, online }) => {
...
// ✅ PERF: NewGroupModal moved OUTSIDE ChatList so it's not re-created every render
const NewGroupModal = memo(({ allUsers, onClose, onCreate }) => {
      ...
      <div style={{
        ...
        <div style={{ maxHeight: 200, overflowY: "auto", marginBottom: 16 }}>
          {allUsers.map((u) => {
            ...
            return (
              <div key={u._id} onClick={() => toggle(u)} style={{
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            onClick={() => onCreate(groupName.trim(), groupMembers)}
...
// ✅ PERF: Individual list items memoized
const DmItem = memo(({ chat, currentUser, onlineUsers, selectedId, onClick }) => {
  ...
  const online = onlineUsers.includes(String(other._id));
  ...
  return (
    ...
    onMouseLeave={(e) => { if (!active) e.currentTarget.style.background = active ? "rgba(124,58,237,0.25)" : "transparent"; }}>
      <Avatar src={other.avatar || ""} name={name} size={44} online={online} />
...
const GroupItem = memo(({ group, currentUser, selectedId, onClick }) => {
...
const UserRow = memo(({ user, onlineUsers, onClick }) => {
  ...
  return (
    ...
    onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}>
      <Avatar src={user.avatar} name={userName(user)} size={40} online={online} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: 14, color: "#fff",
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {userName(user)}
        ...
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>
          @{user.username}
          {online && <span style={{ color: "#22c55e", marginLeft: 6 }}>● Online</span>}
        </div>
      </div>
...
export default function ChatList({ open, currentUser, onlineUsers = [] }) {
  ...
  const loadDms = useCallback(async () => {
    try {
      ...
      dmMap.current.clear();
      data.forEach(chat => {
        const key = (chat.members || []).map(m => String(m._id || m)).sort().join(":");
  ...
  // ✅ PERF: Real-time socket update instead of polling
  useEffect(() => {
    const onReceive = (msg) => {
      if (msg.isGroup || msg.groupId) {
        ...
        setDms(prev => {
          const updated = prev.map(d => {
            const involved = (d.members || []).some(m =>
  ...
  const openChat = useCallback((raw) => {
  ...
  const displayList = () => {
    if (searchResults !== null) return (
      <div>
        {sectionLabel("Users — click to message")}
        {searchResults.length === 0
          ? <div style={{ padding: "16px 12px", color: "rgba(255,255,255,0.3)", fontSize: 14 }}>No users found</div>
          : searchResults.map((u) => (
              <UserRow key={u._id} user={u} onlineUsers={onlineUsers} onClick={() => startDm(u)} />
            ))}
      </div>
    );
        ...
        {dms.length > 0 && (<>
          ...
          {dms.map((chat) => (
            <DmItem
              ...
              onClick={() => openChat({ ...chat, memberDetails: chat.members })}
            />
          ))}
        </>)}
    ...
    return (
      <div>
        <div onClick={() => setShowNewGroup(true)} style={{
        ...
        {groups.length === 0
          ...
          : groups.map((g) => (
              <GroupItem
                ...
                currentUser={currentUser}
                selectedId={selectedId}
                onClick={() => openChat({ ...g, isGroup: true })}
              />
            ))}
      </div>
    );
